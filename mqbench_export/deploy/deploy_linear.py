...
class LinearQuantizer_process(object):
    ...
    def remove_fakequantize_and_collect_params(self, onnx_path, model_name, backend, qmin_max_dict, quant_min=-128, quant_max=127):
        ...
        for node in graph.node:
            if node.op_type in ALL_FAKEQUANTIZER:
                try:
                    ...
                    logger.info(f'No next node for <{node.name}>!')
            ...
            if node.op_type in _FAKEQUANTIZER and 'axis' in parse_attrs(node.attribute):
                ...
                try:
                    ...
                    logger.info(f'qmin and qmax are not found for <{node.name}>!, default qmin={quant_min}, qmax={quant_max} for <{node.name}>!')
                    qmin = quant_min
                    qmax = quant_max
        ...
        if backend == 'vitis':
            logger.info(f"To finish xmodel converting process, call \
                $ mqbench_export.deploy.convert_xir -Q {onnx_filename} -C {onnx_path} -N <name> \
                    in the mqbench_export docker built from Dockerfile")
