...
import mqbench_export.nn.intrinsic as qnni
import mqbench_export.nn.intrinsic.qat as qnniqat
import mqbench_export.nn.qat as qnnqat
from mqbench_export.utils.registry import register_convert_function
from mqbench_export.utils.fusion import fuse_deconv_bn_eval
from mqbench_export.quantization.default_bias_fake_quant import bias_fake_quantizer
...
def convert_qnniqat_linearbn(model, fused_node):
    ...
    # Apply QAT conversion on the BN-merged linear (fused_linear) instead of
    # the original linear, so that the merged weight retains weight_fake_quant.
...
def convert_qnniqat_convbn(model, fused_node):
    """mqbench_export.nn.intrinsic.qat module add bias quant.
